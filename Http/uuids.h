...
#ifndef INTERVIEW_CPP_UUIDS_H
#define INTERVIEW_CPP_UUIDS_H
...
namespace uuids {
	namespace detail {
		...
		constexpr inline unsigned char hex2char(TChar const ch) {
			...
			if (ch >= static_cast<TChar>('a') && ch <= static_cast<TChar>('z'))
				return ch - static_cast<TChar>('a');
			if (ch >= static_cast<TChar>('A') && ch <= static_cast<TChar>('Z'))
				return ch - static_cast<TChar>('A');
			return 0;
		}
	...
	class uuid {
		...
		public:
			...
			explicit uuid_const_iterator(pointer ptr, size_t index) : ptr(ptr), index(index) {}
		...
		class uuid_iterator : public uuid_const_iterator {
	...
	private:
		...
		friend bool operator==(uuid const &, uuid const &)noexcept;
	...
	std::string to_string(uuid const &d) {
		std::stringstream ss;
		ss << d;
		return ss.str();
	}

	std::wstring to_wstring(uuid const &d) {
