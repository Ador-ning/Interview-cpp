...
namespace http {
	...
	request::request() : buffer_{static_cast<char *>(std::malloc(8192)), 0, 8192} {}

	request::~request() {
		if (multipart_parser_)
			multipart_parser_fee(multipart_parser_);
		std::free(buffer_.buffer);
	}

	int request::parser_header(std::size_t last_len) {
		num_headers_ = sizeof(headers_) / sizeof(headers_[0]);
		header_size_ = phr_parse_request(buffer_.buffer, buffer_.size_, &method_, &method_len_, &url_, &url_len_,
		...
		if (header_size_ > 0) {
			http_parser_url_init(&url_info);
			if (http_parser_parse_url(url_, url_len_, false, &url_info) != 0)
				return -1;

			if (url_info.field_set & (1 << UF_QUERY)) {

				kv_parser(url_ + url_info.field_data[UF_QUERY].off,
				          url_ + url_info.field_data[UF_QUERY].off + url_info.field_data[UF_QUERY].len,
				          queries_, '=', '&', true, false);
			}
		}
		return header_size_;
	...
	namespace parser {
		...
		request::form_parts_t::content_disposition_t parse_content_disposition(std::string str) {
			...
			request::form_parts_t::content_disposition_t res;
			res.content_type = get_content_type(start, end);
			while (start != end) {
	...
	bool request::parser_form_multipart() {
		if (!multipart_parser_) {
			// get boundary
			...
			// call back functions --> lambda function
			static const multipart_parser_settings multipart_parser_settings_ = {
					...
					[](multipart_parser *p, const char *at, size_t length) {
						...
						if (part.state_ == 1) {
							if (iequal(part.curr_field.data(), part.curr_field.size(), "Content-Disposition",
							           19)) {
								part.content_disposition_ = parser::parse_content_disposition(part.curr_value_);
							}

							part.meta_.emplace_back(part.curr_field, part.curr_value_);
							part.state_ = 0;

							part.curr_field.clear();
							part.curr_value_.clear();
						}

						part.curr_field += std::string(at, length);
						return 0;
					},
...
// 			multipart_notify_cb on_part_data_begin;
					[](multipart_parser *p, const char *at, size_t len) {
						auto self = static_cast<request *>(multipart_parser_get_data(p));
...
// 			multipart_notify_cb on_headers_complete;
	...
	bool request::parser_form_urlencoded() {
		std::string val, key;
		bool is_k = true;
		auto body_str = body();
	...
	boost::string_ref request::get_header(const char *name, size_t size) const {
		auto it = std::find_if(headers_, headers_ + num_headers_, [this, name, size](struct phr_header const &hdr) {
			return iequal(hdr.name, hdr.len, name, size);
		});
	...
	std::vector<boost::string_ref> request::get_headers(const char *name, size_t size) const {
		...
		for (std::size_t i = 0; i < num_headers_; ++i) {
			if (iequal(headers_[i].name, headers_[i].len, name, size))
				headers.emplace_back(boost::string_ref(headers_->value, headers_->value_len));
		}
	...
	std::vector<request::header_t> request::get_headers() const {
		std::vector<request::header_t> headers;
		for (std::size_t i = 0; i < num_headers_; ++i) {
			headers.emplace_back(header_t{
					boost::string_ref(headers_[i].name, headers_[i].len),
	...
	boost::string_ref request::get_header_cs(std::string const &name) const {
		auto it = std::find_if(headers_, headers_ + num_headers_, [&name](struct phr_header const &hdr) {
			return boost::string_ref(hdr.name, hdr.len) == name;
		});
	...
	std::vector<boost::string_ref> request::get_headers_cs(std::string const &name) const {
		std::vector<boost::string_ref> headers;
		for (std::size_t i = 0; i < num_headers_; ++i) {
			if (boost::string_ref(headers_[i].name, headers_[i].len) == name) {
	...
	void request::increase_buffer(std::size_t size) {
		...
		for (auto it = &headers_[0]; it != headers_ + num_headers_; ++it) { // pointer
