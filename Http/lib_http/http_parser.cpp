...
/* Tokens as defined by rfc 2616. Also lower cases them.
...
// Macro
#define XXP(meth, pos, ch, new_meth) \
...
reexecute:
		switch (CURRENT_STATE()) {
			...
			case s_req_method: {
				...
				if (ch == ' ' && matcher[parser->index] == '\0') {
					...
					switch (parser->method << 16 | parser->index << 8 | ch) {
						XXP(POST, 1, 'U', PUT)
						XXP(POST, 1, 'A', PATCH)
						XXP(CONNECT, 1, 'H', CHECKOUT)
						XXP(CONNECT, 2, 'P', COPY)
						XXP(MKCOL, 1, 'O', MOVE)
						XXP(MKCOL, 1, 'E', MERGE)
						XXP(MKCOL, 2, 'A', MKACTIVITY)
						XXP(MKCOL, 3, 'A', MKCALENDAR)
						XXP(SUBSCRIBE, 1, 'E', SEARCH)
						XXP(REPORT, 2, 'B', REBIND)
						XXP(POST, 1, 'R', PROPFIND)
						XXP(PROPFIND, 4, 'P', PROPPATCH)
						XXP(PUT, 2, 'R', PURGE)
						XXP(LOCK, 1, 'I', LINK)
						XXP(UNLOCK, 2, 'S', UNSUBSCRIBE)
						XXP(UNLOCK, 2, 'B', UNBIND)
						XXP(UNLOCK, 3, 'I', UNLINK)
			...
			case s_headers_almost_done: {
				...
				parser->upgrade = ((parser->flags & (F_UPGRADE | F_CONNECTION_UPGRADE)) ==
...
const char *http_method_str(enum http_method m) {
	return ELEM_AT(method_strings, m, "<unkwon>");
}
...
void http_parser_setting_init(http_parser_settings *settings) {
