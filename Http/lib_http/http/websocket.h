...
#ifndef INTERVIEW_CPP_WEBSOCKET_H
#define INTERVIEW_CPP_WEBSOCKET_H
...
namespace http {
	namespace websocket {
		...
		public:
			...
			static void upgrade_to_websocket(request const &req, response &rep, boost::string_ref const &client_key,
			                                 ws_config_t cfg);
			...
			void
			async_send_msg(const char *data, std::size_t length, opcode_t opCode, async_write_msg_callback_t handler);
		...
		private:
			...
			inline bool
			consume_message(T pay_length, char *&src, std::size_t &length, frame_format_t frame, void *user) {
				if (get_opcode(frame)) {
				...
				if (int(pay_length) <= int(length - MESSAGE_HEADER)) {
					...
					unmask_imprecise_copy_mask(src, src + MESSAGE_HEADER, src + MESSAGE_HEADER - 4,
					                           static_cast<std::size_t>(pay_length));
					if (handle_fragment(src, static_cast<std::size_t>(pay_length), 0, opcode[(unsigned char) op_stack],
					                    is_fin(frame), user)) {
						return true;
					}
					...
					handle_fragment(src, length - MESSAGE_HEADER, remaining_bytes, opcode[(unsigned char) op_stack],
					                is_fin(frame), user);
					return true;
				}
			}
			...
			bool
			handle_fragment(char *data, size_t length, std::size_t remaining_bytes, int opcode, bool fin, void *user);

			std::vector<boost::asio::const_buffer>
			format_message(const char *src, size_t length, opcode_t opcode/*, size_t reportedLength, bool compressed*/);
...
#endif //INTERVIEW_CPP_WEBSOCKET_H
