...
namespace http {
	...
	bool response::to_buffers(std::vector<boost::asio::const_buffer> &buffers) {
		if (!header_buffer_wroted_) {
			...
			buffers.emplace_back(status_string::to_buffer(status_));
	...
	response response::stock_reply(http::response::status_type status) {
		response rep;
		...
		rep.headers_[1].name = "Content_Type";
	...
	void response::reset() {
		...
		content_.clear();
		fs_.reset();
		content_gen_ = {}; // std::string (void) type function
	}

	void response::set_status(http::response::status_type status) {
		status_ = status;
	}

	std::vector<http::response::header_t> &response::headers() {
		return headers_;
	}


	std::vector<http::response::header_t> const &response::headers() const {
		return headers_;
	}
