...
namespace http {
	namespace websocket {
		...
		void websocket_connection::start() {
			...
			conn_->async_read_some(buffer_.data() + LONG_MESSAGE_HEADER, buffer_.size() - LONG_MESSAGE_HEADER,
			                       [self, this](boost::system::error_code const &ec, std::size_t length) {
				                       if (ec) {
					                       if (cfg_.on_error) {
		...
		void websocket_connection::async_send_msg(const char *data, std::size_t length, opcode_t opCode,
		                                          async_write_msg_callback_t handler) {
...
// 			char close_payload[MAX_CLOSE_PAYLOAD + 2];
			...
			std::size_t close_payload_length = format_close_payload(close_payload.get(), static_cast<uint16_t>(code),
			                                                        message, length);
			auto self = this->shared_from_this();
			async_send_msg(close_payload.get(), close_payload_length, opcode_t::CLOSE,
			               [close_payload, self, this](boost::system::error_code const &ec) {
				               if (ec) {
					               conn_->close();
				               } else {
		...
		void
		websocket_connection::unmask_imprecise_copy_mask(char *dst, char *src, char *mask_ptr, std::size_t length) {
		...
		http::websocket::websocket_connection::close_frame_t
		websocket_connection::parse_close_payload(char *src, size_t length) {
			close_frame_t cf = {};
			if (length >= 2) {
...
parseNext:
				for (frame_format_t frame; length >= SHORT_MESSAGE_HEADER;) {
					...
					if ((rsv1(frame) && !set_compressed(user)) || rsv23(frame) ||
					    (get_opcode(frame) > 2 && get_opcode(frame) < 8) ||
					    get_opcode(frame) > 10 ||
					    (get_opcode(frame) > 2 && (!is_fin(frame) || payload_length(frame) > 125))) {
						force_close(user);
						return;
					}

					if (payload_length(frame) < 126) {
						if (consume_message<SHORT_MESSAGE_HEADER, uint8_t>(payload_length(frame), src, length, frame,
						                                                   user)) {
							return;
						}
						...
						if (length < MEDIUM_MESSAGE_HEADER) {
							break;
						} else if (consume_message<MEDIUM_MESSAGE_HEADER, uint16_t>(ntohs(*(uint16_t *) &src[2]), src,
						                                                            length, frame, user)) {
							return;
						}
					} else if (length < LONG_MESSAGE_HEADER) {
						break;
					} else if (consume_message<LONG_MESSAGE_HEADER, uint64_t>(be64toh(*(uint64_t *) &src[2]), src,
					                                                          length, frame, user)) {
						return;
					}
				}
		...
		bool websocket_connection::consume_continuation(char *&src, std::size_t &length, void *user) {
			if (remaining_bytes <= length) {
				...
				if (handle_fragment(src, length, remaining_bytes, opcode[(unsigned char) op_stack], last_fin != 0,
				                    user)) {
					return false;
				}
		...
		bool websocket_connection::handle_fragment(char *data, size_t length, std::size_t remaining_bytes, int opcode,
		                                           bool fin, void *user) {
			if (opcode < 3) {
...
// 						}
						...
						if (cfg_.on_message) {
							cfg_.on_message(this->shared_from_this(), boost::string_ref(data, length),
							                (opcode_t) opcode);
						}
						if (is_closed() || is_shutting_down()) {
							return true;
				...
				if (!remaining_bytes && fin) {
					if (opcode == CLOSE) {
						...
						if (opcode == PING) {
							auto self = this->shared_from_this();
							async_send_msg(control_buffer, opcode_t::PONG,
