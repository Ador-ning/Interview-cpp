	...
	private:
		...
		public:
			explicit param_container(params_t params, request const &req, response &res, context_container ctx)
					: params_(params), req_(req), res_(res), ctx_(ctx) {}
			...
			std::size_t remain_size() { return params_.size() - cur_index_; }

			void set_error() { success_ = false; }
		...
		template<typename Function, class Signature=Function, size_t N = http::function_traits<Signature>::arity>
		struct invoker;
		...
		struct invoker {
			...
			static inline bool call(const Function &func, param_container &parser, const Args &args) {
				...
				typedef typename http::function_traits<Signature>::template args<N - 1>::type arg_type;
				using _G = _get_param_t<typename std::decay<arg_type>::type>;
				auto param = _G::get_param(parser);
				if (!parser.success()) {
					return false;
				}
				return router::invoker<Function, Signature, N - 1>::call(func, parser,
				                                                         std::tuple_cat(std::make_tuple(param), args));
			}
		};
	...
	public:
		...
		void route(const std::string &path, const FunctionT &f) {
			...
			handlers_.emplace_back(_splited_path_handler_t
					                       {
							                       std::move(sp),
							                       std::bind(&invoker<FunctionT>::template call<std::tuple<>>, f,
