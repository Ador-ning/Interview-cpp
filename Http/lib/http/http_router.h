...
#include "../../lib_http/http/request.h"
#include "../../lib_http/http/response.h"
...
namespace http {
	...
	constexpr auto
	type_to_name(std::integral_constant<httpmethod, httpmethod::CONNECT>) noexcept { return "CONNECT"sv; }

	constexpr auto
	type_to_name(std::integral_constant<httpmethod, httpmethod::OPTIONS>) noexcept { return "OPTIONS"sv; }
	...
	private:
		...
		void register_handler_impl(std::string_view name, Type T::* f, T1 &&t, Ap &&... ap) {
			if constexpr(sizeof...(Is) > 0) {
				...
				register_member_func(std::string(name.data(), name.length()), f, std::forward<T>(t),
				                     std::forward<Ap>(ap)...);
			}
		}
		...
		void register_nonmember_func(const std::string &name, Function &&f, Ap &&... ap) {
			this->map_invokers_[name] = [this, f = std::move(f), &ap...](const request &req, response &res) {
				warper(std::make_tuple(ap...), [&f](const request &req1, response &res1) { return f(req1, res1); }, req,
				       res);
			};
		}
		...
		void register_member_func(const std::string &name, Function f, Self &&self, Ap &&... ap) {
			this->map_invokers_[name] = [this, f, &self, &ap...](const request &req, response &res) {
				warper(std::make_tuple(ap...),
				       [&f, &self](const request &req1, response &res1) { return (self.*f)(req1, res1); }, req, res);
			};
		}

		template<typename... AP, typename Function, typename... Args>
		auto warper(std::tuple<AP...> tp, Function &&f, Args &&... args) {
			using result_type = typename http::function_traits<Function>::result_type;
